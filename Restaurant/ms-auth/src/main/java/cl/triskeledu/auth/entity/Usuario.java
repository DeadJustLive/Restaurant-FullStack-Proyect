 ...
 *
 * TABLA EN BD: `usuarios` (PostgreSQL — base de datos `auth`)
 *
 ...
 *   2. Se crea Usuario asociado a la credencial ID 42 en la misma transacción o en un endpoint de perfil.
 *   3. En el frontend, al hacer login, el JWT incluye `sub=42`.
 *      El frontend puede consultar el perfil con GET /api/v1/usuarios/credencial/42.
 *
 * INVARIANTES:
...
public class Usuario {
    ...
    /**
     * ATRIBUTO: credencial
     * Tipo: Entidad UserCredential (FK a tabla user_credentials)
     * Rol: Vínculo entre identidad y perfil. Relación OneToOne real.
     *      Garantiza 1-a-1.
     * Riesgo: CRÍTICO. No modificar. Si se cambia, el usuario pierde acceso a su perfil.
     *         Una vez creado, este campo es INMUTABLE.
     */
    @OneToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "credencial_id", nullable = false, updatable = false)
