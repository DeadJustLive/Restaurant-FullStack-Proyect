...
block Bloque delimitado por
llaves { foo(); bar(); } , { x(); y(); z() }
item ít fn foo() {} , struct Bar; , use std::io;
meta Dentro del atributo cfg!(windows) , doc="comment"
tt Árbol simbólico + , foo , 5 , [?!(???)]
Tenga en cuenta que un comentario doc /// comment se trata de la misma forma que
#[doc="comment"] a una macro.
...
Seguir set
Algunos especificadores de fragmentos requieren que el token que sigue debe ser uno de un
conjunto restringido, denominado "conjunto de seguimiento". Esto permite cierta flexibilidad para
que la sintaxis de Rust evolucione sin romper las macros existentes.
...
Esto expandirá todas las macros y luego imprimirá el resultado expandido en la salida estándar,
por ejemplo, lo anterior probablemente generará:
